---
title: "2026-04-25-Hermes-agent-高级玩法"
created: "2026-04-25"
tags: ["AI","Agent","Article","Automation","Code","Daily","Git","Memory","Note","Project","OpenClaw"]
category: "Articles"
published: true
---
# Hermes agent 高级玩法
...
这意味着：
- 在 CLI 里，`.` 会被解析成“你启动 hermes 时所在的当前目录”。
- 在 Telegram / gateway 里，如果没有显式设置 `terminal.cwd`，会优先看 `MESSAGING_CWD`，否则退回到 Home 目录。
- 所以如果不固定目录，Telegram 场景下默认工作空间容易漂移到家目录，而不是你的项目目录。
...
建议修改完成后重启 gateway，使 Telegram 侧也统一使用这个目录：
```bash
hermes gateway restart
```
